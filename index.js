...
var crypto = require('@dwebjs/crypto')
...
Feed.revelationKey = crypto.revelationKey
...
Feed.prototype[inspect] = function(depth, opts) {
    ...
    return 'DDatabase(\n' +
        ...
        indent + '  revelationKey: ' + opts.stylize((this.revelationKey && pretty(this.revelationKey)), 'string') + '\n' +
...
Feed.prototype._open = function(cb) {
    ...
    function onopen(err, state) {
        ...
        function onsignature(_, sig) {
            ...
            if (!writable && self.writable) return cb(new Error('dDatabase is not writable'))
...
Feed.prototype.download = function(range, cb) {
    ...
    if (!range) range = {}
    if (!cb) cb = noop
    if (!this.readable) return cb(new Error('dDatabase is closed'))
...
Feed.prototype.createReadStream = function(opts) {
    ...
    function read(size, cb) {
        ...
        if (!self.readable) return cb(new Error('dDatabase is closed'))
...
Feed.prototype._onclose = function() {
    ...
    while (this._waiting.length) {
        this._waiting.pop().callback(new Error('dDatabase is closed'))
    }
    while (this._selections.length) {
        this._selections.pop().callback(new Error('dDatabase is closed'))
    }
...
Feed.prototype._append = function(batch, cb) {
    ...
    if (!this.writable) return cb(new Error('This dDatabase is not writable. Did you create it?'))
